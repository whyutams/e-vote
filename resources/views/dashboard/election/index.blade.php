    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg rounded-lg">
        ...
        <div class="p-5 flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between">
            <div class="flex flex-column">
                <div class="mb-3 me-2">
                    <a href="{{ url('/dashboard/elections/create') }}"
                ...
                <tbody>
                    @forelse($elections as $index => $election)
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="{{ $election->status == 'draft' ? 'bg-blue-100 text-blue-800' : ($election->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }} text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm">{{ $election->status == 'draft' ? 'Belum dimulai' : ($election->status == 'active' ? 'Sedang berlangsung' : 'Ditutup') }}</span>
                            </td>
                            <td class="px-6 py-4">
